...
derived_table: {
    sql: select *, trim(split_part(name,'-',1)) as release, trim(split_part(name,'-',2)) as sprint, trim(split_part(name,'-',3)) as team from connectors.jira.sprint where name ~ 'R[0-9]+\s*-\s*S[0-9]+\s*-\s*.*'
      ;;
  }
